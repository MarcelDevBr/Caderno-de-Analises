...
nb1_cells = [
    ...
    code_cell("cols_opiniao = ['bloco_d_estatais', 'bloco_d_tabelamento', 'bloco_e_punitivismo', 'bloco_e_educacao', 'bloco_g_corrupcao', 'bloco_g_pesquisa', 'bloco_g_politica_externa']\n\nfig, axes = plt.subplots(3, 3, figsize=(16, 12))\naxes = axes.flatten()\n\nfor i, col in enumerate(cols_opiniao):\n    sns.histplot(df[col], bins=5, kde=True, ax=axes[i], color='royalblue')\n    axes[i].set_title(col)\n    axes[i].set_xticks([1, 2, 3, 4, 5])\n\nfor j in range(len(cols_opiniao), len(axes)):\n    fig.delaxes(axes[j])\n\nplt.tight_layout()\nplt.show()"),
...
nb2_cells = [
    ...
    code_cell("df_eleitores = pd.read_csv('../data/raw/mock_voters.csv')\n\nvetores_eleitores = df_eleitores[cols_opiniao].head(5).values\nvetores_politicos = df_politicos[cols_opiniao].values\nvetores_partidos = df_partidos[cols_opiniao].values\n\n# --- 2.1 Distância Euclidiana ---\n# Quanto MENOR, mais próximo\ndist_pol = euclidean_distances(vetores_eleitores, vetores_politicos)\ndist_part = euclidean_distances(vetores_eleitores, vetores_partidos)\n\nprint('Distância Euclidiana (Eleitor 0 aos Políticos):')\nprint(pd.Series(dist_pol[0], index=df_politicos.index).sort_values())\n\nprint('\\nDistância Euclidiana (Eleitor 0 aos Partidos):')\nprint(pd.Series(dist_part[0], index=df_partidos.index).sort_values())"),
    md_cell("## 3. Redução de Dimensionalidade (PCA)\nNosso espaço tem 7 dimensões (as 7 perguntas). Para visualizar num gráfico 2D, aplicamos o Principal Component Analysis (PCA) para comprimir as informações preservando a maior variância possível."),
    code_cell("pca = PCA(n_components=2)\n\n# Concatenar todos\nvetores_totais = np.vstack([df_politicos[cols_opiniao].values, df_partidos[cols_opiniao].values, df_eleitores[cols_opiniao].values])\nproj_2d = pca.fit_transform(vetores_totais)\n\nlen_pol = len(df_politicos)\nlen_part = len(df_partidos)\nproj_pol = proj_2d[:len_pol]\nproj_part = proj_2d[len_pol:len_pol+len_part]\nproj_eleit = proj_2d[len_pol+len_part:]\n\nplt.figure(figsize=(10, 8))\n# Plotar amostra de eleitores (fundo)\nplt.scatter(proj_eleit[:, 0], proj_eleit[:, 1], alpha=0.15, color='gray', label='Eleitores')\n\n# Plotar Políticos\ncores_pol = ['red', 'blue', 'orange', 'purple']\nfor i, (idx, row) in enumerate(df_politicos.iterrows()):\n    plt.scatter(proj_pol[i, 0], proj_pol[i, 1], color=cores_pol[i], s=200, edgecolors='black', label=f'Político: {idx}')\n\n# Plotar Partidos\nfor i, (idx, row) in enumerate(df_partidos.iterrows()):\n    plt.scatter(proj_part[i, 0], proj_part[i, 1], color=cores_pol[i], s=200, edgecolors='black', marker='s', alpha=0.5, label=f'Partido: {idx}')\n\nplt.title('Bússola Eleitoral 2026: Projeção 2D (PCA) de Candidatos, Partidos e Eleitores')\nplt.xlabel('Componente Principal 1')\nplt.ylabel('Componente Principal 2')\nplt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')\nplt.tight_layout()\nplt.show()")
]
